...
class PlanResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        return [
            'id'                       => $this->id,
            'title'                    => $this->title,
            'duration'                 => $this->mealTypes->first()->pivot->duration,
            'day_number'               => $this->mealTypes->first()->pivot->day_number,
            'serving_per_food_types'   => $this->servingPerFoodType($this->id,$this->mealTypes->first()->pivot->duration, $this->mealTypes->first()->pivot->day_number),
            'meal_types'               => $this->whenLoaded('mealTypes', MealTypeResource::collection($this->mealTypes))
        ];
    }
