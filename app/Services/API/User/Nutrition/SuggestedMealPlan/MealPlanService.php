...
//        app(CalculateFoodExchangesMeasurementsForMasterServingService::class)->execute($user,$recipes,$servingPerFoodType);
    ...
    public function servingPerFoodType($user, $starches, $fruits, $vegetables, $meats, $dairy, $oils) : array
    {
        ...
        return [
            'Starches'        => $starches !=0 ? round($masterServing->starches ) : $starches,
            'Fruits'          => $fruits !=0 ? round($masterServing->fruits ) : $fruits,
            'Vegetables'      => $vegetables !=0 ? round($masterServing->vegetables ) : $vegetables,
            'Meats'           => $meats !=0 ? round($masterServing->meats ) : $meats,
            'Dairy'           => $dairy !=0 ? round($masterServing->dairy ) : $dairy,
            'Oils'            => $oils !=0 ? round($masterServing->oils ) : $oils,
        ];
    }
